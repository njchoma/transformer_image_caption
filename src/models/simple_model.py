...
class Simple_Model(nn.Module):
    def __init__(self, dict_size, image_feature_dim, vocab, tf_ratio):
        ...
        self.lstm1 = Attention_LSTM(WORD_EMB_DIM,
                                    ...
                                    image_feature_dim,
                                    NB_HIDDEN_LSTM1)
        self.predict_word = Predict_Word(NB_HIDDEN_LSTM1, dict_size)

        self.h = torch.nn.Parameter(torch.zeros(1, NB_HIDDEN_LSTM1))
        self.c = torch.nn.Parameter(torch.zeros(1, NB_HIDDEN_LSTM1))
    ...
    def forward_one_step(self, state, current_word, v_mean, image_feats):
        ...
        h, c = self.lstm1(h, c, image_feats, word_emb)
        y = self.predict_word(h)
        state = [h, c]
        return y, state
    ...
    def init_inference(self, nb_batch, cuda):
        ...
        h = self.h.repeat(nb_batch, 1)
        c = self.c.repeat(nb_batch, 1)
        state = [h, c]
...
class Attention_LSTM(nn.Module):
    def __init__(self, dim_word_emb, nb_image_feats, dim_image_feats, nb_hidden):
        super(Attention_LSTM,self).__init__()
        self.lstm_cell = nn.LSTMCell(nb_image_feats*dim_image_feats+dim_word_emb,
                                     nb_hidden,
                                     bias=True)
        
    def forward(self, h, c, image_feats, word_emb):
        nb_batch = image_feats.size(0)
        image_feats = image_feats.view(nb_batch, -1)
        input_feats = torch.cat((image_feats, word_emb),dim=1)
        h_out, c_out = self.lstm_cell(input_feats, (h, c))
        return h_out, c_out
